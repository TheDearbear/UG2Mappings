...
internal class SectionPolygon : IChunkReader
{
    ...
    public void Deserialize(BinarySpan span, List<VisibleSection> sections)
    {
        ...
        while (span.Pointer < span.Length)
        {
            ...
            var invertor = new Vector2(-1, 1);

            Vector2 minPoint = span.ReadStruct<Vector2>() * invertor;
            Vector2 maxPoint = span.ReadStruct<Vector2>() * invertor;
            Vector2 center = span.ReadStruct<Vector2>() * invertor;

            for (int i = 0; i < pointsCount; i++)
                polygon[i] = span.ReadStruct<Vector2>() * invertor;
